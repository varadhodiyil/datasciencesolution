...
app.controller("postCtrl", function ($scope, $http, fileUpload) {
    ...
    $scope.submit = function (isValid) {
        if (isValid) {
            $http.post(apiURL + "service/loyalty", this.CLV).then(function (result) {
                $(document).ready(function () 
                {
                        divolte.signal("CLV_Submit",{"queryString":$scope.CLV});
                });
                $scope.summary = result.data;
                chart_Data = result.data.data[0];
                ...
                function drawChart(d) {
                    ...
                    dataTable.addColumn('number', 'Purchase Amount'); 
                    d.forEach(function(element) 
                    {
                        if(element.Purchase == "Amount")
                        {
                            dataTable.addRow([element.Month, Number (parseFloat(element.Value).toFixed(2)  )]);
                        }
                        // console.log
                    }, this);
